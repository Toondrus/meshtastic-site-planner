...
## Новая функция: Импорт логов телеметрии (CSV/Excel)

В эту версию добавлена функция загрузки реальных логов телеметрии Meshtastic прямо на карту. Это позволяет удобно сравнивать расчетную зону покрытия с реальными данными тестирования на местности.

### Особенности:
- **Поддержка `.csv` и `.xlsx`**: Можно загружать логи, экспортированные напрямую из узлов Meshtastic или сторонних утилит.
- **Цветовая индикация RSSI**: Точки приема (RX) отображаются цветными маркерами в зависимости от качества сигнала (Зеленый: отлично, Желтый: хорошо, Оранжевый: слабо, Красный: очень слабо).
- **Подробная информация**: При клике на точку отображается имя отправителя, точное время, RSSI, SNR, дистанция и пресет LoRa.
- **Автоматическое центрирование**: При загрузке файла карта автоматически масштабируется и перемещается в район, где проводилось тестирование.

### Как использовать:
1. Запустите проект (инструкции ниже в разделе "Building").
2. Откройте боковое меню **Site Parameters** (Параметры сайта).
3. Нажмите на выпадающее меню **Import Log** (Импорт логов).
4. Загрузите ваш файл `.csv` или `.xlsx` с координатами.
5. Карта автоматически сфокусируется на точках приема пакетов.

*Примечание: Парсер поддерживает как старые форматы заголовков (например, `rx pos lat`, `sender name`), так и новые форматы из CSV (например, `r_lat`, `t_name`). Строки с нулевыми или некорректными координатами автоматически фильтруются.*
